...
class AgreementInjectionTest < Capybara::Rails::TestCase
  ...
  test 'GobDigital User can inject new Agreement' do
    ...
    find("#select2-agreement_service_provider_organization_id-container").click
    find_all(".select2-results__option")[3].click

    page.execute_script('$("input[type=checkbox]").show()')

    assert_content('servicio_2')
    page.find('label', text: 'servicio_2').click

    click_button 'Crear Nuevo Convenio'
    assert_content('Convenio creado correctamente')
  end
